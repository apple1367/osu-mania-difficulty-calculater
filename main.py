...
def get_diff(file):
    ...
    song_len = int(HitObjects[-1]) + 10000
    ...
    for timings in Timing:

        chunklen = float(timings[1])*4

        for edge in chunk:
            if edge > float(timings[0]):
                chunk.remove(edge)

        if timings[1] not in chunk:
            chunk.append(float(timings[0]))

        while float(chunk[-1]) < song_len:
    ...
    for i in range(len(chunk)):
        ...
        if i == 0:
            ...
            chunk_value.append(float(first_timing[1])*4)
    ...
    weighted_group = []
    for i in range(len(group_cal)-1):
